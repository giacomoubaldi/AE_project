...
void TDAQServerBase::dataFlux(){
  ...
  if( fastAcceptor->start()==0 ){
    ...
    if (fastStream != NULL) {
      ...
      m_cb->clear(); // clean up whatever is on the circular buffer
      do {

        // initialise timeout 
        int timeout = 500;   // 0.5 ms waiting time
...
void *TDAQServerBase::producer(){
  ...
  while( n>0 && p_server->bytesAvailable()>0){
    p_server->readData(evt, 20000); // read a chunk of data (max 20000?) from FIFO
    evt.erase(evt.begin(),evt.end());
    n--;
  }
